---
title: "Tutorial for package `dynmod`"
...
can be solved with package `dynmod`.
...
`dynmod` uses the same mod file as Dynare. However, only the `var`, `varexo`,
...
The function `create_mod` creates a `DynMod` object from 
the mod file.

```{r}
mdl <- create_mod("islm.mod")
mdl
```
