...
R objects, including S3 or S4 objects, does not apply to R6 classes.
...
report. The function `solve_steady()` employs package `nleqslv` to solve for
...
The methods `set_endo_value()` and `set_exo_value` are usefull for simple 
experiments,
but for a more typical applications the exogenous shocks and values of the
