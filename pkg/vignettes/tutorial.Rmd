---
title: "Tutorial for package `dynmdl`"
...
The function `create_mdl` creates a `DynMdl` object from 
the mod file.

```{r}
mdl <- create_mdl("islm.mod")
mdl
```
...
The methods `set_endo_value()` and `set_exo_values()` are useful for simple 
experiments,
