%$fit$
varexo c__av_hu_ca ; parameters sigma_c__av_hu_ca ; sigma_c__av_hu_ca = 0.01 ;
...
trend_var(growth_factor = 1 + groei_a) asi;
trend_var(growth_factor = 1 + groei_h) hi;
...
log(pl_e) = log(py) + log(y) - log(ld) - .36 + 0*(1- p_lk0 - p_u) - p_pl_u * u;     % (8)loonvergelijking
 ...
 b_e  = p_b0 * bi*hi * asi * (pb/pmi)^-p_b_pb ;                  % (10) uitvoer	, asi trend var toegevoegd
...
% consumption  
  cd  = (p_cd - p_cd_lead)  * log(c_e/c_e(-1)) + p_cd_lead  * log((c_e(+1)/c_e)) + (1 - p_cd )  * p_groei_y  - p_c_ecm * log(c(-1)/c_e(-1)) + c__av_hu_ca  ;
  cd  = log(c /c(-1));   
gap_c = log(c /c_e);
...
% loonvoet
  pld  = (p_pld - p_pld_lead)  * log(pl_e/pl_e(-1)) + p_pld_lead  * log(pl_e(+1)/pl_e) + (1 - p_pld )  * (p_groei_h + p_groei_p) - p_pl_ecm * log(pl(-1)/pl_e(-1))   ;
  pld  = log(pl /pl(-1));   
gap_pl = log(pl /pl_e);
