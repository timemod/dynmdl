...
test_that("get_equations works correctly", {
  ...
  expect_known_output(cat_eqs(eqs), "expected_output/eqs.txt")
  
  eqs_fit <- mdl_fit$get_equations()
  expect_known_output(cat_eqs(eqs_fit), "expected_output/eqs_fit.txt")
...
test_that("example with static and dynamic tags", {
  ...
  expect_known_output(cat_eqs(mdl$get_equations()), 
                      "expected_output/eqs_static_tags.txt")
  expect_known_output(cat_eqs(mdl$get_original_equations()), 
