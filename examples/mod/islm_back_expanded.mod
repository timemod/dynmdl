% A backward looking version of the ISLM model

%Declaring variables
$EXPA(co;1/2)
var y_co yd_co t_co c_co i_co md_co r_co;  % endogenous variables
...
yd = y - t;
t = t0 + t1 * y;
c = c0 + c1 * yd(-1) + c2 * yd + c3 * yd(-2) + c4 * r + c5 * r^2;
i = i0 + i1 * y(-1) + i2 * y + i3 * y(-2) + i4 * r + i5 * r^2;
